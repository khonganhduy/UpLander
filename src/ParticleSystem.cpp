...
void ModelParticleSystem::update() {
	// check if empty and just return
	if (modelParticles.size() == 0) return;

	vector<ModelParticle>::iterator p = modelParticles.begin();
	vector<ModelParticle>::iterator tmp;

	// check which particles have exceed their lifespan and delete
	// from list.  When deleting multiple objects from a vector while
	// traversing at the same time, we need to use an iterator.
	//
	while (p != modelParticles.end()) {
		if (p->lifespan != -1 && p->age() > p->lifespan) {
			tmp = modelParticles.erase(p);
			p = tmp;
		}
		else p++;
	}
	...
	// update forces on all particles first 
	//
	for (int i = 0; i < modelParticles.size(); i++) {
		for (int k = 0; k < forces.size(); k++) {
			if (!forces[k]->applied)
				forces[k]->updateForce(&modelParticles[i]);
		}
	}
	...
	//
	for (int i = 0; i < modelParticles.size(); i++) {
		modelParticles[i].integrate();
	}

}

void ModelParticleSystem::add(const ModelParticle &particle) {
	modelParticles.push_back(particle);
}
void ModelParticleSystem::draw() {
	for (int i = 0; i < modelParticles.size(); i++) {
		modelParticles[i].update();
		modelParticles[i].draw();
	}
}
...
JumpImpulseForce::JumpImpulseForce(float magnitude) {
	this->magnitude = magnitude;
	applyOnce = true;
}

void JumpImpulseForce::updateForce(Particle* particle) {
	particle->forces += dir * magnitude;
}

void JumpImpulseForce::setDir(ofVec3f newDir) {
	dir = newDir;
}


MoveImpulseForce::MoveImpulseForce(float magnitude) {
	this->magnitude = magnitude;
	applyOnce = true;
}

void MoveImpulseForce::updateForce(Particle* particle) {
	particle->forces += dir * magnitude;
}

void MoveImpulseForce::setDir(ofVec3f newDir) {
	dir = newDir;
}